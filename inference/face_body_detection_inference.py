...
class FaceBodyDetectionInference:
    ...
    def calculate_density(self, faces, bodies, image_shape):
        """Calculate crowd density analysis based on people count"""
        h, w = image_shape[:2]
        total_area = h * w
        
        # Jumlah orang (berdasarkan body detection)
        num_people = len(bodies)
        
        # Simple density: jumlah orang per area
        people_density = num_people / (total_area / 1000000)  # orang per juta pixel
        
        # Crowd level berdasarkan jumlah orang
        if num_people == 0:
            crowd_level = "Empty"
            crowd_intensity = 0
        elif num_people <= 2:
    ...
    def _draw_stats_panel(self, image, faces, bodies, density_info):
        ...
        texts = [
            ...
            f"Face-to-Body Ratio: {density_info['face_body_ratio']:.2f}"
        ]
    ...
    def process_image(self, image_path, output_path=None, show_result=False):
        ...
        print(f"📈 Densities - People: {density_info['people_density']:.1f} orang/juta pixel")
